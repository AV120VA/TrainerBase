...
function UserCommunities() {
  ...
  const rawCommunities = useMemo(
    () => (userCommunities ? Object.values(userCommunities) : []),
    [userCommunities]
  );

  useEffect(() => {
    dispatch(getUserCommunities()).then(() => setIsLoaded(true));
  }, [dispatch]);
      ...
      {isLoaded && (
        <div className="communities-list-page-container">
          <div className="community-list-header-and-button">
            <h2 className="community-list-header">Manage Communities</h2>
            <button
